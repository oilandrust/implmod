...
void display()
{
	...
	if(editMode == RIG && showBalls){
		for(size_t i = 0 ; i < skeleton.primitives.size(); i++){
			skeleton.primitives[i]->renderGuizmo();
		}
	}
	...
	if(rigEditor->selectedBone){
		rigEditor->selectedBone->renderWorldSpace(true);
		if(showBalls){
			for(size_t i = 0; i < rigEditor->selectedBone->primitives.size(); i++){
				rigEditor->selectedBone->primitives[i]->renderGuizmo();
			}
		}
	}
...
int main(int argc, char **argv)
{
	...
	rigEditor = new RigEditor(bones);

	editors.push_back(mainEditor);
	editors.push_back(rigEditor);
	mainEditor->addEditor(rigEditor);
