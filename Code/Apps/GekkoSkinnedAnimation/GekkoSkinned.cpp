...
namespace
{
	...
	Mat4x4f boneInvTrans[50];
	Mat4x4f boneRestTrans[50];
	Vec3f   boneRestP2[50];
...
void enableRayTraceNoBvh(){
	...
	if(reloadShader)
	{
		...
		vs = create_glsl_shader(GL_VERTEX_SHADER, "shaders", "raytraceBall.vert");
		fs = create_glsl_shader(GL_FRAGMENT_SHADER, "shaders", "raytraceTubeInterpAndBallTex.frag");
	...
	glUniform1i(glGetUniformLocation(prog_traceRaster,"nbOfBones"), nbOfBones-1);
	glUniformMatrix4fv(glGetUniformLocation(prog_traceRaster,"boneInvTrans"), bones.size()-1, false, boneInvTrans[0].get());
	glUniformMatrix4fv(glGetUniformLocation(prog_traceRaster,"boneRestTrans"), bones.size()-1, false, boneRestTrans[0].get());
...
void loadAnimation(const string& filename){
	...
	clip->setTargetSkeleton(bones);
	animation->saveRestPose(bones);
...
int main(int argc, char **argv)
{
	...
	loadAndInitializeSkeleton(&skeleton,&implicitFuntion,"TRexAnim1.skl");
	loadAnimation("TRexAnim1.skl");
	animation->play();



	for(size_t i = 0; i < bones.size()-1; i++){
		bones[i+1]->addPrimitive(new MetaTube(Vec3f(0),Vec3f(0,0,bones[i+1]->length),0.1f,0.1f));
		boneRestTrans[i] = bones[i+1]->worldOrientation.get_Mat4x4f();
		boneRestTrans[i][3] = Vec4f(bones[i+1]->worldPosition,1);
		boneRestP2[i] = bones[i+1]->worldPosition + bones[i+1]->worldOrientation.inverse().apply(Vec3f(0,0,1))*bones[i+1]->length;
	}
