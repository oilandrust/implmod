...
bool getClosestPointOnRayFromTube(vec3 origine, vec3 dir, int tubeIndex, out vec3 highest){
	...
	float maxval = -1;
	for(int i = 0; i < hit; i++){
		float dist = implicitMetaTube(hitPtsInNode[i], p1s[tubeIndex], p2s[tubeIndex], radius[tubeIndex] );
		if( dist > maxval){
			closest = i;
			maxval = dist;
		}
	}
	highest = hitPtsInNode[closest];
	return true;
}
...
bool rayIntersectsSurfaceInNode(vec3 origine, vec3 dir, int node){
	...
	for(int i = 0; i < 4; i++){
		int t = intCompFromIndex(i,nodeT1);
		if( 0 <= t){
			if(getClosestPointOnRayFromTube(origine,dir,t,highest)){
					hitPtsInNode[hit] = highest;
					hit++;
			}
		}else
			break;
...
/*
	...
	*/
	...
	float maxval = -10000;
	for(int i = 0; i < hit; i++){
		float dist = implicitFunctionOfNode(hitPtsInNode[i], node );
		if( dist > maxval){
			closest = i;
			maxval = dist;
		}
	}
	posPoint = hitPtsInNode[closest];
	if(maxval > 0)
		return true;
