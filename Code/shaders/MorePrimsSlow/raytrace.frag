...
void main()
{
	...
	while(node != -1){
		...
		vec3 old;	
		ray = getFirstRayBoxInter(origine, rayDir, node) + ds*rayDir;
		while( !done && steps < maxSteps && PointInsideNode(ray,node)){
		...
		if(implicitFunctionOfNode(ray,node) > 0){
			/*
			vec3 low = old;
		...
		}else{//No intersection found in the node, continue BVH traversal
			node = RayIntestectsBVH2(origine, rayDir);
		}
	}	
