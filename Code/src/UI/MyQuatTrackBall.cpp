...
namespace GLGraphics
{
    ...
    MyQuatTrackBall::MyQuatTrackBall(const Vec3f& _centre, 
			 float _eye_dist,
			 unsigned _width, 
			 unsigned _height):
    ...
    void MyQuatTrackBall::grab_ball(MyTrackBallAction act, const Vec2i& v)
    {
    ...
    void MyQuatTrackBall::roll_ball(const Vec2i& v)
    {
    if(v[0] < 0 || v[0] >= static_cast<int>(width)
       || v[1] < 0 || v[1] >= static_cast<int>(height))
      return;      
    ...
    void MyQuatTrackBall::set_position(const Vec2f& _last_pos) 
    {
	stop_spin();
	last_pos = _last_pos;
    }
    ...
    void MyQuatTrackBall::do_spin() 
    {
	qrot = qrot*qinc;
    }
    
    // Cease any rotational glide by zeroing the increment.
    void MyQuatTrackBall::stop_spin() 
    {
	qinc.set(0.0, 0.0, 0.0, 1.0);
    }
    
    void MyQuatTrackBall::rotate(const Vec2f& new_v) 
    {
	calcRotation(new_v);
	do_spin();	
    }
    
    void MyQuatTrackBall::pan(const Vec2f& new_v) 
    {
	trans += (new_v - last_pos) * Vec2f(eye_dist);
    }
    
    void MyQuatTrackBall::zoom(const Vec2f& new_v) 
    {
	eye_dist += (new_v[1] - last_pos[1]) * eye_dist;
	eye_dist = max(eye_dist, 0.01f);
    }

	void MyQuatTrackBall::lookDownAxis(CGLA::Axis axis)
	{
		/*
		switch(axis){
			case(XAXIS):
				target = target.make_rot(Vec3f(0,0,1),Vec3f(1,0,0));
				break;
			case(YAXIS):
				target = target.make_rot(Vec3f(0,0,1),Vec3f(1,0,0));
				break;
			case(ZAXIS):
				target = target.identity_Quatf();
				break;
		}
		*/
	}
    
    void MyQuatTrackBall::calcRotation(const Vec2f& new_pos) 
    {
...
*/
    ...
    float MyQuatTrackBall::projectToSphere(const Vec2f& v) 
    {
#ifndef M_SQRT_2
    ...
    Vec2f MyQuatTrackBall::scalePoint(const Vec2i& v) const
    {
    ...
    void MyQuatTrackBall::get_view_param(Vec3f& eye, Vec3f& _centre, Vec3f& up) const
    {
    ...
    void MyQuatTrackBall::set_gl_modelview() const
    {
    ...
    bool MyQuatTrackBall::is_spinning() const
    {
