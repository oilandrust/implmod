...
void Skeleton::removeChild(Skeleton* child){
	vector<Skeleton*>::iterator it = remove(this->children.begin(),this->children.end(),child);
	this->children.erase(it,this->children.end());
}

void Skeleton::addPrimitive(MetaPrimitive* primitiv){
	if(primitiv->skeleton != 0){
		primitiv->skeleton->removePrimitive(primitiv);
	}

	this->primitives.push_back(primitiv);
	primitiv->skeleton = this;
	primitiv->_updateWorldGeometry();
	primitiv->_boneLengthChanged();
}

void Skeleton::removePrimitive(MetaPrimitive* primitive){
	vector<MetaPrimitive*>::iterator it = remove(this->primitives.begin(),this->primitives.end(),primitive);
...
void load(Skeleton& skeleton, std::ifstream& ifs){
	...
	std::reverse(skeleton.children.begin(),skeleton.children.end());
}